...
test('NEGATIVE: Basic MCP server initialization and health check', async (t) => {
    ...
    try {
        ...
        t.false(toolsText.includes('Invalid request parameters'), 'Should not have initialization timing errors');
...
test('NEGATIVE: Verify critical stdio servers are accessible', async (t) => {
    ...
    try {
        ...
        t.true(foundServers.length >= 2, `At least 2 critical stdio servers should be accessible. Found: ${foundServers.join(', ')}`);
        // Most importantly, should not have timing errors
        t.false(toolsText.includes('Invalid request parameters'), 'Critical servers should not have timing errors');
    }
    finally {
        if (transport?.stop) {
            await transport.stop();
...
//# sourceMappingURL=chatgpt-simulation-negative.test.js.map