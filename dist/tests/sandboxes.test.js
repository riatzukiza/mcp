...
const branchNameFor = (sandbox) => sandbox?.branch;
...
test("listSandboxes enumerates created sandboxes", async (t) => {
    ...
    const alpha = sandboxes.find((entry) => entry.id === "alpha");
    t.is(branchNameFor(alpha), branchNameFor(sandboxA));
    const beta = sandboxes.find((entry) => entry.id === "beta");
...
test("removeSandbox deletes git worktree", async (t) => {
    ...
    await t.throwsAsync(() => removeSandbox({ repoPath: repo, sandboxId: "gamma" }), { message: /does not exist/u });
});
