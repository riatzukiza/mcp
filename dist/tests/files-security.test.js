...
const createFile = async (filePath, content = 'test content') => {
...
test('writeFileContent: prevents symlink escape via direct symlink', async (t) => {
    ...
    await t.throwsAsync(() => writeFileContent(sandbox, 'escape.txt', 'malicious content'), { message: /symlink escape detected/ });
...
test('writeFileContent: prevents symlink escape via parent directory symlink', async (t) => {
    ...
    await t.throwsAsync(() => writeFileContent(sandbox, 'malicious/secret.txt', 'malicious content'), { message: /symlink escape detected|parent symlink escape detected/ });
    // Verify the outside file wasn't modified
    const originalContent = await fs.readFile(outsideFile, 'utf8');
    t.is(originalContent, 'secret content');
});
test('writeFileContent: prevents symlink escape via nested path components', async (t) => {
    ...
    await t.throwsAsync(() => writeFileContent(sandbox, 'level1/escape/secret.txt', 'malicious content'), { message: /symlink escape detected/ });
    // Verify the outside file wasn't modified
    const originalContent = await fs.readFile(outsideFile, 'utf8');
    t.is(originalContent, 'secret content');
});
test('writeFileContent: prevents symlink escape via relative path symlink', async (t) => {
    ...
    await t.throwsAsync(() => writeFileContent(sandbox, 'subdir/escape/secret.txt', 'malicious content'), { message: /symlink escape detected/ });
    // Verify the outside file wasn't modified
    const originalContent = await fs.readFile(outsideFile, 'utf8');
    t.is(originalContent, 'secret content');
});
...
test('writeFileContent: prevents path traversal attempts', async (t) => {
    ...
    for (const maliciousPath of maliciousPaths) {
        await t.throwsAsync(() => writeFileContent(sandbox, maliciousPath, 'malicious content'), { message: /path outside root|symlink escape detected/ });
    }
});
test('writeFileLines: includes same symlink protection', async (t) => {
    ...
    await t.throwsAsync(() => writeFileLines(sandbox, 'escape.txt', ['malicious content'], 1), { message: /symlink escape detected/ });
    // Verify the outside file wasn't modified
    const originalContent = await fs.readFile(outsideFile, 'utf8');
    t.is(originalContent, 'secret content');
});
...
//# sourceMappingURL=files-security.test.js.map