...
test('security: path validation blocks all traversal attempts', async (t) => {
    ...
    t.true(blockedPaths.length > maliciousInputs.length * 0.8, `Should block >80% of malicious inputs, blocked ${blockedPaths.length}/${maliciousInputs.length}`);
    ...
    for (const path of criticalPaths) {
        ...
        t.true(result.securityIssues.some((issue) => issue.includes('traversal') || issue.includes('absolute')), `Should identify traversal issue in: ${path}`);
    }
});
test('security: unicode homograph attacks are detected', async (t) => {
    ...
    for (const attack of unicodeAttacks) {
        ...
        t.true(result.securityIssues.some((issue) => issue.includes('traversal') || issue.includes('unicode')), `Should detect unicode issue in: ${attack}`);
    }
});
test('security: glob pattern attacks are prevented', async (t) => {
    ...
    for (const attack of globAttacks) {
        ...
        t.true(result.securityIssues.some((issue) => issue.includes('glob') || issue.includes('traversal')), `Should detect glob issue in: ${attack}`);
    }
});
// ============================================================================
...
test('security: file operations prevent symlink escapes', async (t) => {
    ...
    try {
        ...
        t.true(err.message.includes('symlink') || err.message.includes('security'), 'Error should mention symlink or security');
    }
    ...
    try {
        ...
        for (const path of maliciousPaths) {
            try {
                ...
                const err = error;
                t.true(err instanceof Error, 'Should throw Error instance');
                t.true(err.message.includes('security') || err.message.includes('outside'), `Error should mention security for path: ${path}`);
            }
        }
    ...
    try {
        ...
        for (const injection of injectionAttempts) {
            try {
                ...
                t.true(err.message.includes('validation') || err.message.includes('security'), `Should fail with validation/security error for: ${injection}`);
            }
        }
    }
    finally {
...
test('security: authorization enforces role-based access', async (t) => {
    ...
    try {
        ...
        const err = error;
        t.true(err.message.includes('Authorization denied'), 'Should deny access with authorization error');
    }
...
test('security: file content validation prevents malicious uploads', async (t) => {
    ...
    for (const content of maliciousContents) {
        ...
        const isMalicious = content.includes('<script>') ||
...
test('security: comprehensive attack simulation', async (t) => {
    ...
    try {
        ...
        t.true(blockedAttacks >= attacks.length * 0.7, `Should block >70% of attacks, blocked: ${blockedAttacks}/${attacks.length}`);
    }
    finally {
        if (originalRoot) {
            process.env.MCP_ROOT_PATH = originalRoot;
...
test('security: configuration validation prevents insecure settings', async (t) => {
    ...
    for (const config of insecureConfigs) {
        ...
        const isSecure = (config.rateLimitMaxRequests || 1000) <= 10000 &&
...
test('security: security measures do not significantly impact performance', async (t) => {
    ...
    t.true(avgValidationTime < 10, `Average validation time should be <10ms, was: ${avgValidationTime}ms`);
