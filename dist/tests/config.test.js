...
test('resolveStdioTools unions endpoint tools when top-level empty', (t) => {
    ...
    const result = resolveStdioTools(cfg);
    t.deepEqual(new Set(result), new Set(['github_request', 'files_list_directory', 'files_view_file']));
});
test('resolveStdioTools normalizes endpoint tool identifiers', (t) => {
    ...
    const result = resolveStdioTools(cfg);
    t.deepEqual(new Set(result), new Set(['pnpm_run_script', 'pnpm_remove', 'process_get_task_runner_config', 'process_stop']));
});
