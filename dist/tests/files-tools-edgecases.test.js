...
test('filesListDirectory - handles very long directory paths', async (t) => {
    ...
    const tool = filesListDirectory(createMockContext());
    const result = await tool.invoke({ rel: path.relative(tempDir, currentPath) });
    t.true(result.ok);
    t.true(result.entries.some((e) => e.name === 'deep.txt'));
});
...
test('filesTreeDirectory - handles very deep directory structures', async (t) => {
    ...
    const result = await tool.invoke({ rel: tempDir, depth: 1000 });
    t.true(result.ok);
...
test('filesViewFile - handles binary files gracefully', async (t) => {
    ...
    const result = await tool.invoke({ relOrFuzzy: path.join(tempDir, 'binary.bin') });
    t.true(result.ok);
...
test('filesViewFile - handles files with very long lines', async (t) => {
    ...
    const tool = filesViewFile(createMockContext());
    const result = await tool.invoke({ relOrFuzzy: path.join(tempDir, 'longline.txt') });
    t.true(result.ok);
...
test('filesViewFile - handles Unicode and special characters', async (t) => {
    ...
    const tool = filesViewFile(createMockContext());
    const result = await tool.invoke({ relOrFuzzy: path.join(tempDir, 'unicode.txt') });
    t.true(result.ok);
...
test('filesViewFile - handles very large files', async (t) => {
    ...
    const tool = filesViewFile(createMockContext());
    const result = await tool.invoke({ relOrFuzzy: path.join(tempDir, 'large.txt') });
    t.true(result.ok);
...
test('filesViewFile - handles files with no line endings', async (t) => {
    ...
    const tool = filesViewFile(createMockContext());
    const result = await tool.invoke({ relOrFuzzy: path.join(tempDir, 'nolinebreaks.txt') });
    t.true(result.ok);
...
test('filesWriteFileContent - handles very large content', async (t) => {
    ...
    const result = await tool.invoke({ filePath, content: largeContent });
    t.true(result.ok);
...
test('filesWriteFileContent - handles content with null bytes', async (t) => {
    ...
    const tool = filesWriteFileContent(createMockContext());
    const result = await tool.invoke({ filePath, content: contentWithNulls });
    t.true(result.ok);
    t.true(fs.existsSync(filePath));
...
test('filesWriteFileContent - handles file paths with special characters', async (t) => {
    ...
    const tool = filesWriteFileContent(createMockContext());
    const result = await tool.invoke({ filePath, content: 'Special content' });
    t.true(result.ok);
    t.true(fs.existsSync(filePath));
    const readContent = fs.readFileSync(filePath, 'utf8');
...
test('filesSearch - handles very long search patterns', async (t) => {
    ...
    const result = await tool.invoke({ query: longQuery, rel: tempDir });
    t.true(result.ok);
...
test('filesSearch - handles files with mixed line endings', async (t) => {
    ...
    const tool = filesSearch(createMockContext());
    const result = await tool.invoke({ query: 'Line', rel: tempDir });
    t.true(result.ok);
...
test('files tools handle resource exhaustion gracefully', async (t) => {
    ...
    const result = await listTool.invoke({ rel: tempDir });
    t.true(result.ok);
...
//# sourceMappingURL=files-tools-edgecases.test.js.map