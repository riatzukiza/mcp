...
test("NEGATIVE: Debug output must be filtered out from JSON-RPC messages", (t) => {
    ...
    for (const debugMessage of debugMessages) {
        t.false(isValidJsonRpcMessage(debugMessage), `Debug message should be filtered out: ${JSON.stringify(debugMessage)}`);
    }
});
test("NEGATIVE: Invalid JSON-RPC structures must be rejected", (t) => {
    ...
    for (const invalidMessage of invalidMessages) {
        t.false(isValidJsonRpcMessage(invalidMessage), `Invalid JSON-RPC should be rejected: ${JSON.stringify(invalidMessage)}`);
    }
});
test("POSITIVE: Valid JSON-RPC messages must be accepted", (t) => {
    ...
    for (const validMessage of validMessages) {
        t.true(isValidJsonRpcMessage(validMessage), `Valid JSON-RPC should be accepted: ${JSON.stringify(validMessage)}`);
    }
});
test("NEGATIVE: Malformed JSON-RPC with debug-like content must be rejected", (t) => {
    ...
    for (const malformedMessage of malformedDebugLikeMessages) {
        t.false(isValidJsonRpcMessage(malformedMessage), `Malformed debug-like message should be rejected: ${JSON.stringify(malformedMessage)}`);
    }
});
test("NEGATIVE: Common stdio debug patterns must be filtered", (t) => {
    ...
    for (const debugPattern of commonDebugPatterns) {
        t.false(isValidJsonRpcMessage(debugPattern), `Common debug pattern should be filtered: ${debugPattern}`);
    }
});
...
//# sourceMappingURL=debug-filtering-unit.test.js.map