...
const mkCtx = (endpoints, allToolIds, proxySources = { inline: [], config: [], fallback: [] }) => ({
    env: {},
...
test('validate-config flags unknown workflow tool ids', async (t) => {
    ...
    const result = ResultSchema.parse(await tool.invoke(undefined));
    t.false(result.ok);
    t.true(result.errors.some((msg) => msg.includes('files_apply')), result.errors.join('\n'));
...
test('validate-config detects workflow references missing from endpoint', async (t) => {
    ...
    const result = ResultSchema.parse(await tool.invoke(undefined));
    t.false(result.ok);
    t.true(result.errors.some((msg) => msg.includes('files_search')), result.errors.join('\n'));
