...
import { CONFIG_FILE_NAME, ConfigSchema, loadConfigWithSource, saveConfigFile, } from '../config/load-config.js';
...
test('loadConfigWithSource identifies explicit config file', (t) => {
    ...
    const { config: loaded, source } = loadConfigWithSource(env, ['node', 'test', '--config', configPath], dir);
...
test('loadConfigWithSource accepts absolute paths outside cwd', (t) => {
    ...
    const env = Object.create(null);
    const { config: loaded, source } = loadConfigWithSource(env, ['node', 'test', '--config', configPath], runnerCwd);
    t.deepEqual(loaded.tools, ['exec_run']);
    t.is(source.type, 'file');
    if (source.type === 'file') {
        t.is(source.path, configPath);
...
//# sourceMappingURL=config-write.test.js.map