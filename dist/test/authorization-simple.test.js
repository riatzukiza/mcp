...
import { createAuthorizedToolFactory, getCurrentAuthConfig, validateAdminIp, getAuthorizationHealth, } from '../core/authorization.js';
...
const createMockTool = (name) => () => ({
    ...
    invoke: async (args) => ({ tool: name, args, success: true }),
});
// Mock tool context
...
describe('Authorization Core Functionality', () => {
    ...
    it('should report authorization health', () => {
        ...
        assert(typeof health.requireAuthForDangerous === 'boolean', 'Should report require auth dangerous');
    ...
    it('should enforce strict mode for unknown tools', async () => {
        ...
        try {
            ...
            assert(error.message.includes('not found in authorization configuration'), 'Should deny unknown tools in strict mode');
        }
    });
    ...
    it('should require auth for dangerous operations', async () => {
        ...
        try {
            ...
            assert(error.message.includes('Authentication required for dangerous operation'), 'Should require auth for dangerous operations');
        }
    });
...
//# sourceMappingURL=authorization-simple.test.js.map