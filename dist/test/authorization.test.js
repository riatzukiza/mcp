...
import { createAuthorizedToolFactory, getToolAuthRequirements, getDangerousTools, getToolsByPermissionLevel, auditLogger, getCurrentAuthConfig, isStrictModeEnabled, isAuthRequiredForDangerous, isAdminIpWhitelisted, validateAdminIp, getStrictModeDeniedTools, getAuthorizationHealth, } from '../core/authorization.js';
// Mock tool factory for testing
const createMockTool = (name, shouldFail = false) => {
    return () => ({
        spec: {
            name,
...
describe('Authorization Framework', () => {
    ...
    describe('Tool Authorization Requirements', () => {
        ...
        it('should categorize tools by permission level', () => {
            ...
            assert(deleteTools.includes('kanban_delete_task'), 'kanban_delete_task should require delete permission');
        });
    });
    describe('Guest User Access', () => {
        it('should allow read-only operations', async () => {
            ...
            const toolFactory = createAuthorizedToolFactory(createMockTool('files_view_file'), 'files_view_file');
            const tool = toolFactory(context);
            const result = (await tool.invoke({ path: 'test.txt' }));
        ...
        it('should deny dangerous operations', async () => {
            ...
            const toolFactory = createAuthorizedToolFactory(createMockTool('files_write_content'), 'files_write_content');
            const tool = toolFactory(context);
            await assert.rejects(() => tool.invoke({ filePath: 'test.txt', content: 'hello' }), /Authorization denied.*Role 'guest' lacks required permission level: write/);
        });
        it('should deny command execution', async () => {
            ...
            const tool = toolFactory(context);
            await assert.rejects(() => tool.invoke({ commandId: 'test' }), /Authorization denied.*Role 'guest' not in required roles: developer, admin/);
        });
    });
    describe('User Access', () => {
        it('should allow write operations', async () => {
            ...
            const toolFactory = createAuthorizedToolFactory(createMockTool('kanban_update_status'), 'kanban_update_status');
            const tool = toolFactory(context);
            const result = (await tool.invoke({ uuid: 'test-uuid', status: 'in-progress' }));
        ...
        it('should deny delete operations', async () => {
            ...
            const toolFactory = createAuthorizedToolFactory(createMockTool('kanban_delete_task'), 'kanban_delete_task');
            const tool = toolFactory(context);
            await assert.rejects(() => tool.invoke({ uuid: 'test-uuid' }), /Authorization denied.*Role 'user' lacks required permission level: delete/);
        });
    });
    describe('Developer Access', () => {
        it('should allow delete operations', async () => {
            ...
            const toolFactory = createAuthorizedToolFactory(createMockTool('kanban_delete_task'), 'kanban_delete_task');
            const tool = toolFactory(context);
            const result = (await tool.invoke({ uuid: 'test-uuid' }));
        ...
        it('should deny admin operations', async () => {
            ...
            const toolFactory = createAuthorizedToolFactory(createMockTool('process_update_task_runner_config'), 'process_update_task_runner_config');
            const tool = toolFactory(context);
            await assert.rejects(() => tool.invoke({ config: {} }), /Authorization denied.*Role 'developer' not in required roles: admin/);
        });
    });
    describe('Admin Access', () => {
        it('should allow all operations', async () => {
            ...
            const adminResult = (await adminTool.invoke({ commandId: 'test' }));
            assert(adminResult.success, 'Admin should be able to execute commands');
            // Test delete tool
            const deleteToolFactory = createAuthorizedToolFactory(createMockTool('kanban_delete_task'), 'kanban_delete_task');
            const deleteTool = deleteToolFactory(context);
            const deleteResult = (await deleteTool.invoke({ uuid: 'test-uuid' }));
    ...
    describe('Error Handling', () => {
        it('should handle tool failures gracefully', async () => {
            ...
            const toolFactory = createAuthorizedToolFactory(createMockTool('failing_tool', true), 'failing_tool');
            const tool = toolFactory(context);
        ...
        it('should handle invalid roles', async () => {
            ...
            const toolFactory = createAuthorizedToolFactory(createMockTool('files_view_file'), 'files_view_file');
            const tool = toolFactory(context);
            await assert.rejects(() => tool.invoke({ path: 'test.txt' }), /Invalid user role: invalid-role/);
        });
    });
    describe('Tool Description Updates', () => {
        it('should add authorization notice to tool descriptions', () => {
            ...
            const tool = toolFactory(context);
            assert(tool.spec.description.includes('[Authorization required]'), 'Tool description should include authorization notice');
        });
    });
    describe('Enhanced Authorization Features', () => {
        ...
        describe('Strict Mode', () => {
            it('should deny unknown tools when strict mode is enabled', async () => {
                ...
                const tool = toolFactory(context);
                await assert.rejects(() => tool.invoke({ test: 'data' }), /Authorization denied.*Tool 'unknown_tool' not found in authorization configuration \(strict mode enabled\)/);
            });
        ...
        describe('Require Auth for Dangerous Operations', () => {
            it('should deny dangerous operations for anonymous users when enabled', async () => {
                ...
                const toolFactory = createAuthorizedToolFactory(createMockTool('files_write_content'), 'files_write_content');
                const tool = toolFactory(context);
                await assert.rejects(() => tool.invoke({ filePath: 'test.txt', content: 'hello' }), /Authorization denied.*Authentication required for dangerous operation 'files_write_content' \(requireAuthForDangerous enabled\)/);
            });
            it('should allow dangerous operations for authenticated users when enabled', async () => {
                ...
                const toolFactory = createAuthorizedToolFactory(createMockTool('kanban_update_status'), // Non-dangerous but requires write
        ...
        describe('Admin IP Whitelist', () => {
            it('should allow admin access from whitelisted IP', async () => {
                ...
                const toolFactory = createAuthorizedToolFactory(createMockTool('exec_run'), 'exec_run');
                const tool = toolFactory(context);
                const result = (await tool.invoke({ commandId: 'test' }));
            ...
            it('should deny admin access from non-whitelisted IP', async () => {
                ...
                const toolFactory = createAuthorizedToolFactory(createMockTool('exec_run'), 'exec_run');
                const tool = toolFactory(context);
                await assert.rejects(() => tool.invoke({ commandId: 'test' }), /Authorization denied.*Admin access denied from IP 192.168.1.200/);
            });
        ...
        describe('Configuration Integration', () => {
            ...
            it('should provide authorization health information', () => {
                ...
                assert(health.requireAuthForDangerous === true, 'Should report require auth dangerous status');
            ...
            it('should return current auth config', () => {
                ...
                assert(typeof config.requireAuthForDangerous === 'boolean', 'Should include requireAuthForDangerous');
    ...
    describe('Backward Compatibility', () => {
        it('should maintain existing role hierarchy', async () => {
            ...
            for (const testCase of testCases) {
                ...
                const toolFactory = createAuthorizedToolFactory(createMockTool(testCase.tool), testCase.tool);
                const tool = toolFactory(context);
                if (testCase.shouldAllow) {
                    ...
                    await assert.rejects(() => tool.invoke({ test: 'data' }), new RegExp(`Authorization denied`), `${testCase.role} should not be able to use ${testCase.tool}`);
                }
            }
        });
    });
