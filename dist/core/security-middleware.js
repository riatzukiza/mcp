...
export class SecurityMiddleware {
    ...
    blockIP(ipAddress, reason, durationMinutes) {
        ...
        console.warn(`[security] IP blocked: ${ipAddress} - ${reason} (expires: ${expiresAt.toISOString()})`);
    }
    // Violation Recording
    async recordViolation(ipAddress, violationType, details) {
        const block = this.ipBlocks.get(ipAddress);
        ...
        if (currentViolations >= this.config.ipBlocking.maxViolations - 1) {
            ...
            if (block) {
                ...
                this.ipBlocks.set(ipAddress, {
                    ipAddress,
                    blockedAt: new Date(),
                    expiresAt: new Date(Date.now() + this.config.ipBlocking.violationDecayHours * 60 * 60 * 1000),
    ...
    async recordSecurityEvent(event) {
        ...
        if (event.severity === 'critical') {
            console.error(`[security] Critical event: ${event.violationType} from ${event.ipAddress}`, event.details);
        }
    }
    ...
    blockIPManually(ipAddress, reason, durationMinutes = 60) {
