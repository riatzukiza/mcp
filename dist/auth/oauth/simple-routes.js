 ...
 */
export function registerSimpleOAuthRoutes(fastify, config) {
    ...
    fastify.get(`/.well-known/oauth-protected-resource/mcp`, async (_request, reply) => {
        return reply.send({
            resource: `${getBaseUrl(_request)}/mcp`,
            authorization_servers: [`${getBaseUrl(_request)}/mcp`],
            scopes_supported: ['read', 'write', 'admin'],
            bearer_methods_supported: ['header'],
            resource_documentation: `${getBaseUrl(_request)}/auth/oauth/docs`,
        });
    });
    ...
    fastify.post(`/.well-known/oauth-registration/mcp`, async (request, reply) => {
        try {
            ...
            const clientStore = global.__oauth_client_store || {};
            ...
            global.__oauth_client_store = clientStore;
    ...
    fastify.post(`${basePath}/login`, async (request, reply) => {
        try {
            ...
            const parsedBody = tryParseJson(request.body);
            const { provider, redirectTo } = parsedBody;
            if (!provider) {
                return reply.status(400).send({
            ...
            const tempStore = global.__oauth_temp_store || {};
            tempStore[state] = { redirectTo };
            global.__oauth_temp_store = tempStore;
    ...
    fastify.get(`${basePath}/login`, async (request, reply) => {
        try {
            ...
            const { response_type, client_id, redirect_uri, state, scope, code_challenge, code_challenge_method, } = query;
            ...
            const dynamicRedirectUri = `${getBaseUrl(request)}/auth/oauth/callback`;
            // Start OAuth flow with the OAuthSystem using the dynamic redirect URI
            const { authUrl: providerAuthUrl, state: oauthState } = config.oauthSystem.startOAuthFlow(provider, dynamicRedirectUri, hasPkce
    ...
    fastify.post(`${basePath}/token`, async (request, reply) => {
        try {
            ...
            const postedRedirectUri = body?.redirect_uri;
            ...
            let provider = null;
            if (clientId && typeof config.oauthSystem.getProviderByClientId === 'function') {
                provider = config.oauthSystem.getProviderByClientId(clientId);
            }
            if (!provider) {
            ...
            let token;
            try {
                token = await config.oauthSystem.exchangeCodeForTokensDirect(provider, code, {
                    codeVerifier,
    ...
    fastify.post(`${basePath}/callback`, async (request, reply) => {
        try {
            ...
            let code, state, error, codeVerifier;
            if (body.grant_type === 'authorization_code') {
                ...
                console.log('[OAuth Callback] Detected ChatGPT PKCE format with code_verifier:', codeVerifier ? 'present' : 'missing');
            }
            else {
            ...
            if (!storedData) {
                console.log('[OAuth Callback] No stored state, processing direct OAuth callback for MCP client');
                ...
                const { redirectTo: storedRedirectTo, originalState: storedOriginalState, clientRedirectUri: storedClientRedirectUri, } = storedData;
                ...
                try {
                    ...
                    if (!clientId || !clientSecret) {
                        return reply.status(500).send({
                            ...
                            message: 'GitHub OAuth credentials not configured. Please set OAUTH_GITHUB_CLIENT_ID and OAUTH_GITHUB_CLIENT_SECRET environment variables.',
                        });
                    }
                    ...
                    console.log('[OAuth Callback] Direct OAuth exchange successful for user:', userData.login);
                }
                catch (error) {
