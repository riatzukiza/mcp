...
export function loadOAuthConfig() {
    ...
    const jwtConfig = {
        ...
        algorithm: getEnv('OAUTH_JWT_ALGORITHM', 'HS256'),
    };
    // User Registry Configuration
    const userRegistryConfig = {
        ...
        defaultRole: getEnv('USER_REGISTRY_DEFAULT_ROLE', 'user'),
    ...
    const integrationConfig = {
        ...
        defaultRole: getEnv('OAUTH_DEFAULT_ROLE', 'user'),
    ...
    const httpConfig = {
        ...
        sameSitePolicy: getEnv('OAUTH_SAME_SITE_POLICY', 'lax'),
    };
    return {
        oauth: oauthConfig,
        jwt: jwtConfig,
...
function loadGoogleProviderConfig() {
    ...
    return {
        ...
        prompt: getEnv('OAUTH_GOOGLE_PROMPT', 'consent'),
    };
}
/**
...
export function validateOAuthConfig(config) {
    ...
    const availableProviders = Object.keys(config.oauth.providers).filter((key) => config.oauth.providers[key] !== undefined);
    ...
    const validTrustedProviders = config.integration.trustedProviders.filter((provider) => availableProviders.includes(provider));
    if (validTrustedProviders.length === 0 && availableProviders.length > 0) {
        throw new Error(`No valid trusted providers configured. Available: ${availableProviders.join(', ')}, Trusted: ${config.integration.trustedProviders.join(', ')}`);
    }
    // Update the trusted providers to only include valid ones
    config.integration.trustedProviders = validTrustedProviders;
 ...
 */
export function getOAuthConfigSummary(config) {
    const availableProviders = Object.keys(config.oauth.providers).filter((key) => config.oauth.providers[key] !== undefined);
    return {
        providers: {
