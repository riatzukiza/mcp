...
export class McpAuthMiddleware {
    ...
    constructor(authManager, mcpAuthorizer, oauthIntegration) {
        ...
        return async (request, reply) => {
            ...
            try {
                ...
                if (!authResult.success) {
                    ...
                    request.authContext = this.createGuestContext(request);
                    request.mcpAuthContext = null;
                    return;
                }
                // 4. Authorization (MCP-specific)
                if (authResult.mcpContext && config.resourcePath) {
                    const authzResult = this.mcpAuthorizer.authorizeResource(authResult.mcpContext, config.resourcePath, {
                ...
                request.mcpAuthContext = authResult.mcpContext;
                request.authWarnings = authResult.warnings || [];
                ...
                console.log(`[AUTH] ${authResult.method} authentication successful for ${authResult.context?.userId} (${Date.now() - startTime}ms)`);
            }
            catch (error) {
                console.error('[AUTH] Middleware error:', error);
                await this.logSecurityEvent(request, 'middleware_error', error.message);
     ...
     */
    async authenticateRequest(request, config) {
    ...
    tryEnvAuth(request) {
        try {
            ...
            const userRole = process.env.MCP_USER_ROLE || 'guest';
            const context = {
                userId,
                role: userRole,
                permissions: [],
     ...
     */
    async createMcpContext(authContext, method) {
     ...
     * Validate session
     */
    validateSession(context, config) {
 ...
 */
export function createAuthMiddleware(authManager, mcpAuthorizer, oauthIntegration) {
