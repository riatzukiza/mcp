...
const main = async () => {
    ...
    const proxies = specs.map((spec) => new StdioHttpProxy(spec, (msg, ...rest) => {
        console.log(`[proxy:${spec.name}] ${msg}`, ...rest);
    }));
    await Promise.all(proxies.map(async (proxy) => {
        await proxy.start();
        console.log(`[proxy:${proxy.spec.name}] started command: ${proxy.spec.command} ${proxy.spec.args.join(" ")}`.trim());
    }));
    ...
    for (const proxy of proxies) {
        ...
        console.log(`[proxy:${proxy.spec.name}] available at http://${options.host}:${options.port}${route}`);
    }
    ...
    await new Promise((resolve, reject) => {
        ...
        server.listen(options.port, options.host, () => {
            console.log(`[proxy] listening on http://${options.host}:${options.port}${prefix || "/"}`);
            resolve();
        });
    });
