...
const ensureCwd = (cwd) => cwd ?? process.cwd();
...
const createGithubGraphqlClient = (ctx) => {
    const endpoint = ctx.env.GITHUB_GRAPHQL_URL ?? 'https://api.github.com/graphql';
    const token = ctx.env.GITHUB_TOKEN;
    if (!token) {
        throw new Error('github.review tools require GITHUB_TOKEN in the environment to call GitHub GraphQL.');
    }
    return async ({ query, variables, schema, }) => {
        ...
        if (!res.ok) {
            throw new Error(`[github.review] GraphQL request failed with ${res.status}: ${JSON.stringify(payload)}`);
        }
...
const getRepositoryId = async (client, owner, name) => {
...
const getPullRequestId = async (client, owner, name, number) => {
    const data = await client({
...
export const githubReviewOpenPullRequest = (ctx) => {
    const shape = {
        owner: z.string(),
        repo: z.string(),
    ...
    return {
        spec: {
            ...
            description: 'Open a new pull request targeting a base branch with the provided title and body.',
...
export const githubReviewGetComments = (ctx) => {
    const shape = {
        owner: z.string(),
        repo: z.string(),
        number: z.number().int(),
    ...
    return {
        ...
        invoke: async (raw) => {
          ...
          repository(owner: $owner, name: $name) {
            ...
            if (!pr) {
                throw new Error(`[github.review] Pull request not found: ${args.owner}/${args.repo}#${args.number}`);
            }
            return pr.comments;
        },
    };
};
export const githubReviewGetReviewComments = (ctx) => {
    const shape = {
        owner: z.string(),
        repo: z.string(),
        number: z.number().int(),
...
export const githubReviewSubmitComment = (ctx) => {
    const shape = {
        owner: z.string(),
        repo: z.string(),
        number: z.number().int(),
...
export const githubReviewRequestChangesFromCodex = (ctx) => {
    const shape = {
        owner: z.string(),
        repo: z.string(),
        number: z.number().int(),
    ...
    const buildCommentBody = (message) => {
    ...
    return {
        spec: {
            ...
            description: 'Request changes from Codex by posting an issue-level pull request comment tagging @codex.',
            inputSchema: Schema.shape,
            stability: 'experimental',
            since: '0.1.0',
        },
...
export const githubReviewSubmitReview = (ctx) => {
    const shape = {
        owner: z.string(),
        repo: z.string(),
        number: z.number().int(),
...
export const githubReviewGetActionStatus = (ctx) => {
    const shape = {
        owner: z.string(),
        repo: z.string(),
        number: z.number().int(),
    ...
    return {
        spec: {
            ...
            description: 'Fetch the latest workflow and check status for the most recent pull request commit.',
            inputSchema: Schema.shape,
            stability: 'experimental',
            since: '0.1.0',
        },
...
export const githubReviewCommit = () => {
    const shape = {
        message: z.string(),
    ...
    return {
        spec: {
            ...
            description: 'Create a git commit. Optionally stage specific paths or use --all/--allow-empty.',
            inputSchema: Schema.shape,
            stability: 'experimental',
            since: '0.1.0',
        },
        invoke: async (raw) => {
            ...
            const commitArgs = ['commit', '--message', args.message];
            if (args.allowEmpty)
...
export const githubReviewPush = () => {
    ...
    return {
        spec: {
            ...
            description: 'Push the current branch to the specified remote with optional upstream/force settings.',
            inputSchema: Schema.shape,
            stability: 'experimental',
            since: '0.1.0',
        },
        invoke: async (raw) => {
            ...
            return { pushed: true, stdout, stderr };
        },
    };
};
export const githubReviewCheckoutBranch = () => {
    const shape = {
        branch: z.string(),
        cwd: z.string().optional(),
    };
...
export const githubReviewCreateBranch = () => {
    const shape = {
        branch: z.string(),
    ...
    return {
        ...
        invoke: async (raw) => {
            const args = Schema.parse(raw);
            const cwd = ensureCwd(args.cwd);
            const gitArgs = ['checkout', '-b', args.branch];
            if (args.startPoint)
                gitArgs.push(args.startPoint);
            const { stdout, stderr } = await runGit(gitArgs, cwd);
            return { branch: args.branch, stdout, stderr };
        },
    };
};
export const githubReviewRevertCommits = () => {
    const shape = {
