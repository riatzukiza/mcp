import { z } from 'zod';
import { fetchPullRequestSummary, fetchPullRequestFiles, pullRequestIdentityShape, PullRequestIdentitySchema, } from './pull-request-api.js';
import { parseUnifiedPatch, resolveNewLinePosition, } from './position-resolver.js';
const mapFileWithHunks = (file) => ({
    path: file.path,
    status: file.status,
...
const buildResolverSuccess = (path, resolution, prefer) => {
    if ('reason' in resolution) {
        return {
            ok: false,
            reason: resolution.reason,
            hint: resolution.reason === 'LINE_OUTDATED_OR_NOT_IN_DIFF'
    ...
    return prefer === 'rest'
        ? { ok: true, rest }
        : { ok: true, graphql, rest };
};
const resolvePositionForFile = (file, line, rangeStart, prefer) => {
...
export const githubPrFiles = (ctx) => {
    const spec = {
...
export const githubPrResolvePosition = (ctx) => {
    const shape = {
...
//# sourceMappingURL=pull-request-data.js.map