...
const toSuccessResult = (bin, args, cwd) => ({
    command: bin,
    args,
    cwd,
    exitCode: 0,
...
const toErrorResult = (bin, args, cwd, error) => {
...
const toStringArray = (value) => (Array.isArray(value) ? value : [value]);
export const normalizeStringList = (value) => toStringArray(value).map((entry) => entry.trim());
...
export const buildPnpmArgs = (command, options = {}) => [
    ...normalizeFilters(options.filter).flatMap((value) => ['--filter', value]),
    ...command,
];
const createExecutor = (ctx) => {
...
const createInstallTool = (ctx, exec = createExecutor(ctx)) => {
...
const createAddTool = (ctx, exec = createExecutor(ctx)) => {
...
const createRemoveTool = (ctx, exec = createExecutor(ctx)) => {
...
const createRunScriptTool = (ctx, exec = createExecutor(ctx)) => {
...
export const pnpmInstall = (ctx) => createInstallTool(ctx);
export const pnpmAdd = (ctx) => createAddTool(ctx);
export const pnpmRemove = (ctx) => createRemoveTool(ctx);
export const pnpmRunScript = (ctx) => createRunScriptTool(ctx);
