...
export function getAuthConfig() {
    ...
    // Basic auth config
    config.defaultRole = process.env.MCP_DEFAULT_ROLE;
    ...
    if (config.userRegistry) {
        ...
        config.userRegistry.sessionTimeout = parseInt(process.env.MCP_USER_REGISTRY_SESSION_TIMEOUT || '3600', 10);
        config.userRegistry.maxSessionsPerUser = parseInt(process.env.MCP_USER_REGISTRY_MAX_SESSIONS || '5', 10);
        config.userRegistry.enableUserSearch = process.env.MCP_USER_REGISTRY_ENABLE_SEARCH !== 'false';
        config.userRegistry.defaultRole =
            process.env.MCP_USER_REGISTRY_DEFAULT_ROLE || config.userRegistry.defaultRole;
