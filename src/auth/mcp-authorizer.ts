...
export class McpAuthorizer {
  ...
  validateAccessToken(token: string): McpAuthContext | McpAuthError {
    try {
      ...
      // Validate role
      const validRoles: UserRole[] = ['guest', 'user', 'developer', 'admin'];
  ...
  authorizeResource(
    ...
    resourcePath: string,
    requirements: McpResourceRequirements,
    request?: FastifyRequest,
  ): { allowed: boolean; error?: McpAuthError } {
    // 1. Check required scopes
    if (requirements.requiredScopes.length > 0) {
      const hasRequiredScopes = requirements.requiredScopes.every((scope) =>
        authContext.scopes.includes(scope),
      );

      if (!hasRequiredScopes) {
        const error: McpAuthError = {
    ...
    if (requirements.maxPermissionLevel) {
      const roleHierarchy: Record<UserRole, number> = { guest: 0, user: 1, developer: 2, admin: 3 };
   ...
   */
  createAuthMiddleware(resourceRequirements: McpResourceRequirements) {
    return async (request: FastifyRequest, reply: FastifyReply): Promise<void> => {
      ...
      const authzResult = this.authorizeResource(
        authResult,
        resourcePath,
   ...
   */
  private sendAuthError(reply: FastifyReply, error: McpAuthError): void {
  ...
  private logAuditEntry(
    ...
    console.log(
      `[MCP-AUTH:${level}] ${entry.timestamp.toISOString()} ${authContext.userId}:${authContext.role} ${resource} ${result}${reason ? ` - ${reason}` : ''}`,
    );
  }
   ...
   */
  getAuditLog(count: number = 100): typeof this.auditLog {
   ...
   */
  getUserAuditLog(userId: string, count: number = 100): typeof this.auditLog {
   ...
   */
  getDeniedAttempts(count: number = 100): typeof this.auditLog {
 ...
 */
export function createMcpAuthorizer(config: McpAuthConfig): McpAuthorizer {
