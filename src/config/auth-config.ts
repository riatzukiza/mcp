...
export function getAuthConfig(): AuthConfig {
  ...
  if (config.oauth) {
    ...
    if (config.oauth.providers.google) {
      ...
      config.oauth.providers.google.prompt = (process.env.OAUTH_GOOGLE_PROMPT as any) || 'consent';
    }
    ...
    config.oauth.jwt.accessTokenExpiry = parseInt(
      process.env.OAUTH_JWT_ACCESS_EXPIRY || '3600',
      10,
    );
    config.oauth.jwt.refreshTokenExpiry = parseInt(
      process.env.OAUTH_JWT_REFRESH_EXPIRY || '2592000',
      10,
    );
    config.oauth.jwt.algorithm = (process.env.OAUTH_JWT_ALGORITHM as any) || 'HS256';
  }
